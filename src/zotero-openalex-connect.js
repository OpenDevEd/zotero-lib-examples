...
const argv = yargs(hideBin(process.argv))
  ...
  .demandOption(['group'])
  .usage(
    'Usage: zotero-openalex-connect.js --group 123 <keys> The script tries to match a zotero item with an openalex item via: an existing openalex id, via the doi, via the title.'
  )
  .help()
  .check((argv) => {
    if (argv._.length === 0) {
